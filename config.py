...
def _validate_config(cfg: AppConfig) -> AppConfig:
    ...
    if cfg.agent.max_tool_iterations <= 0:
        cfg.agent.max_tool_iterations = 25
    if cfg.agent.timeout_seconds <= 0:
        cfg.agent.timeout_seconds = 300
    if cfg.agent.compaction_token_threshold <= 0:
        cfg.agent.compaction_token_threshold = 80000
    if cfg.agent.compaction_keep_recent < 0:
        cfg.agent.compaction_keep_recent = 10
    if cfg.session.history_limit <= 0:
        cfg.session.history_limit = 100
