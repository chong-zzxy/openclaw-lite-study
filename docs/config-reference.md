...
| `compaction_token_threshold` | int | 历史消息 token 总量超过此阈值时触发上下文压缩 |
| `compaction_keep_recent` | int | 压缩时保留最近 N 条消息不压缩 |
| `stream` | bool | 是否启用流式输出（边生成边打印） |

## permissions — 权限配置

| 字段 | 类型 | 说明 |
|------|------|------|
| `mode` | string | 权限模式：`default`（默认询问）、`acceptEdits`（自动允许编辑）、`plan`（只读规划）、`bypassPermissions`（跳过所有权限检查） |
| `auto_allow_read_tools` | bool | 是否自动允许只读工具（如 read、glob、grep）无需确认 |
